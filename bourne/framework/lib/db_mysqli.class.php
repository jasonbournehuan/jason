...
class db_mysqli implements db_interface {
	...
	// 批量 update 指定字段，除非是必要的更新，否则不使用这个函数
	public function update2($table, $sqladd, $data) {
		$tablename = $this->tablepre.$table;
		if(is_array($data)) {
			$s = '';
			foreach($data as $k=>$v) {
			...
			$s = substr($s, 0, -1);
			//echo "UPDATE $tablename SET $s WHERE $sqladd";exit;
			return $this->query("UPDATE $tablename SET $s WHERE $sqladd", $this->wlink);
		} else {
			return FALSE;
		}
	...
	//分组列表详细
	public function group($table, $where = array(), $field = "", $fields = array(), $start = 0, $limit = 0){
		$tablename = $this->tablepre.$table;
		if(empty($fields)){
			$fields = '*';
		}else{
			...
			if(!is_array($v)) {
				...
				foreach($v as $k1=>$v1) {
					if($k1 == 'EXP'){
						...
						$v1 = "%$v1%";
						$s .= "$k $k1 '$v1' AND ";
					}else if($k1 == 'LIKE_LEFT'){
						$k1 = 'LIKE'; 
						$v1 = "%$v1";
						$s .= "$k $k1 '$v1' AND ";
					}else if($k1 == 'LIKE_RIGHT'){
						$k1 = 'LIKE'; 
						$v1 = "$v1%";
						$s .= "$k $k1 '$v1' AND ";
					}else if($k1 == 'OR'){
						...
						$kk = $k;
						$k = $k1 = $v1 = '';
						if(!empty($vv1) && is_array($vv1)){
							$k1 = "(";
							$new_v1_arr = array();
							foreach($vv1 as $k2 => $v2){
								$new_v1_arr[] = $kk."='".$v2."'";
							}
							$new_v1_str = implode(" OR ", $new_v1_arr);
							$k1 .= $new_v1_str.")";
							$s .= "$k1 AND ";
						}
					}else{
	...
	*/
	public function index_fetch_id($table, $keyname, $cond = array(), $orderby = array(), $start = 0, $limit = 0) {
		$s = $this->index_fetch_sql($table, $keyname, $cond, $orderby, $start, $limit);
		//echo $s;exit;
		$return = array();
		$result = $this->query($s, $this->rlink);
		while($data = mysqli_fetch_assoc($result)) {
	...
	//导出数据库文件为文本，方便处理
	public function outfile($table, $keyname, $cond = array(), $orderby = array(), $start = 0, $limit = 0, $file = '') {
		if(!empty($file)){
			if(is_file($file)){
				return -2;//文件存在，避免删除重要文件，不删除操作
			}
			$s = $this->index_fetch_sql($table, $keyname, $cond, $orderby, $start, $limit);
			//数据以|^|分割每个字段，以换行表示每条数据
			$s .= 'into outfile \''.$file.'\' fields terminated by \'|^|\' optionally enclosed by \'\' escaped by \'\' lines terminated by \'\n\' ';
			$return = array();
			$result = $this->query($s, $this->rlink);
			return $result;
		}else{
	...
	public function index_fetch_sql($table, $keyname, $cond = array(), $orderby = array(), $start = 0, $limit = 0) {
		$tablename = $this->tablepre.$table;
	...
	//批量多字段更新，适用于多条数据更新多个不同值数据，建议根据数据长度选择处理数据的数量，只允许主键索引更新，否则效率很低
	//up_data格式 array('主键' => array('更新字段1' => '更新内容1', '更新字段2' => '更新内容2'))
	public function all_update($table, $key, $up_data = array()){
		$tablename = $this->tablepre.$table;
		if(!empty($up_data) && !empty($key)){
			$s = "UPDATE ".$tablename." SET ";
			$id_list = $table_list = $up_list = $sql_set = array();
			foreach($up_data as $k => $v){
				$table_name = array();
				$id_list[] = '\''.$k.'\'';
				foreach($v as $k1 => $v1){
					if(empty($table_list[$k1])){
						$table_list[$k1] = ' '.$k1.' = CASE '.$key.' ';;
					}
					$table_name[$k1] = ' WHEN \''.$k.'\' THEN \''.$v1.'\' ';
				}
				$up_list[$k] = $table_name;
			}
			...
			$s .= implode(',', $table_list);
			unset($table_list);
			$s .= 'WHERE '.$key.' IN ('.implode(',', $id_list).')';
			//echo $s;exit;
			return $this->query($s, '', 1);
		}else{
			return 0;
		}
	}
	...
	*/
	...
	// -------------> 公共方法，非公开接口
	public function query($sql, $link = NULL, $rows = 0) {
