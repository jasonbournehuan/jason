...
class api761 extends base_model{
    ...
    private function get_log_api($stime,$etime,$page = 1,$data = array()){
        $page_num = 1000;
        /*$stime = strtotime("2019-12-03 15:00:00");
        $etime = strtotime("2019-12-03 16:00:00");*/
        $params_arr = array(
            "expids" => [1,2],
            "stime" => $stime,
            "etime" => $etime,
        ...
        /*$function_name = "bet_info_" . $this->game_code($result['d']['data']['0']['kind']);
        $card = $this->$function_name($result['d']['data']['0']);
        print_r($card);exit;
        print_r($result['d']['data']['0']);exit;
        print_r($result);exit;*/
     ...
     * @return array
     */
    public function  bet_info_11($data = array())
    {
            $result = array();
           foreach($data['why']['chairs'] as $key => $value)
           {
               if($value['uid']  ==  $data['uid'] )
               {
                   $result  = array(
                       "bet_info" => array(
                           "player" => $this->card($value['sendcard'])
                       ),
                       "draw_info" => '',
                   );
               }
           }
           $public_card =  $this->card($data['why']['publiccard']);
           $result['bet_info']['public_card'] = $public_card;
           return $result;
    }
    ...
    public function card($data)
    {
        ...
        {
          $len  = strlen($value)-1 >= 1 ? strlen($value)-1 : 1;
          $card = str_split($value,$len);
          $str1 = isset($card['1'])  ? $type[$card['1']] : '';
          $str  = $str1. $number[$card['0']];
          $result .= $str . ",";
        }
