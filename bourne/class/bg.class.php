...
class bg extends base_model{
    ...
    public function reg($num = 1){
       ...
       /* 注册代理账号
        $api_data = array(
            'id' => $this->game_user['uid'],
            'method' => "open.agent.create",
            'random' => $this->password,
            'sign' => md5($this->password.$this->api_sn.$this->uppername.$this->api_secretKey),
            'sn' => $this->api_sn,
            'loginId' => $this->uppername,
            'password' => $this->password
        );*/
//        print_r($this->api_secretKey);exit;
  ...
  /*启用账号*/
        ...
        $data = json_decode($data,true);
            if(!empty($data)){
                if(empty($data['error'])){
                    return array(
                        'status' => 1,
                        'balance' => $data['result'] >=1 ? $data['result'] : 0,
                        'deny_balance' => '0',
                    );
    ...
    public function get_log($game_id = '', $start_time , $end_time , $num = 1 ,$error=''){
       ...
       /* $api_data = array(
            ...
            'random' => $_SERVER['time'],
            'sign' => $this->sign($_SERVER['time']),
            'sn' => $this->api_sn,
            'startTime' => "2019-08-14 11:22:00",
            'endTime' => "2019-08-17 11:22:00",
            'pageIndex' =>1,
            'pageSize' => 500000,
        );*/
        $api_data = array(
