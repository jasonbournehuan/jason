...
class fanya extends base_model{
    ...
    public function get_money($num = 1){
        ...
        if(isset($data['success']) && $data['success'] == 1){
            return array(
                'status' => 1,
                'balance' => intval($data['info']['Money']*100)/100,
                'deny_balance' => '0',
            );
        }else{
            $num += 1;
    ...
    public function get_log($start_time,$end_time, $num = 1){
        ...
        $start_time = strtotime('2019-12-07 19:30:01');
        $end_time = strtotime('2019-12-07 20:00:01');
        $api_data = array(
            ...
            if($data['success'] == 1){
                ...
                if(!empty($data['info']['list'])){
                    foreach($data['info']['list'] as $k => $v){
                        $v['bet_info'] = array(
                                "bet_info" => $v['Category'].'-'.$v['League'].'-'.$v['Match'].'-'.$v['Bet'].'-'.$v['Content'],
                                "draw_info" => $v['Result'],
                                );
                        $return['data'][] = $v;
                    }
