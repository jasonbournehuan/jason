...
class api_control extends common_control {
	...
	function on_goto_game(){
        ...
        $this->$class_name->game_user['order_id'] = $this->order_id;
		...
		if($goto_game['status'] == 1){
            ...
            $log_id = $this->za->login_log($this->_user['id'],$this->_api['site_id'],$this->ip,'','',$goto_game,1,$this->logo_pid,$this->game_id,$this->order_id);
            if(!empty($log_id))
            {
                $goto_game['orders_number'] = $this->encrypt_order($log_id);
            }
            $goto_game['order_id'] = $this->order_id;
		}
	...
	function on_out_game($platform_id = 0, $user = array(), $return = 0){
		...
		if($out_game['status'] == 1){
            ...
            $log_id = $this->za->login_log($this->_user['id'],$this->_api['site_id'],$this->ip,'-'.$out_game['money'],'',$out_game,2,$this->out_platform_id,$this->last_game_id,$this->order_id);
            if(!empty($log_id))
            {
                $out_game['orders_number'] = $this->encrypt_order($log_id);
            }
            $out_game['order_id'] = $this->order_id;
		}
    ...
    private function check_user_parameter()
    {
        ...
        $this->order_id = core::gpc('order_id', 'P');//商户订单号
    ...
    private function on_query_all_balance()
    {
            ...
            $this->c('loginlog');
            $seven =  $_SERVER['time'] - (60*60*24*7) ;
            $user_game_log = $this->loginlog->group( array("typeid" => 1 , "uid" => $this->_api['site_id'].sprintf("%011s",$this->uid),'add_time'=> array(' > ' => $seven)), 'platform_id',array('platform_id','game_id') );
            $user = array(
                'uid' => $this->uid,
                'old_username' => $this->username,
                'site_id' => $this->_api['site_id'],
            ...
            {
                 if(isset($this->_platform_list[$value['platform_id']]))
                 {
                     $class_name =   $this->_platform_list[$value['platform_id']]['class_name'];
                     $this->c($class_name);
                     $this->_user = $this->$class_name->check_user($user);
            ...
            $result = array("status" => 1 , "total_money" => $Total_money , "data" => "" , "order_id" => $this->order_id);
