...
class bbin extends base_model{
	...
	public function reg($num = 1){
		...
		$api_data = array(
			...
			'key' => 'qwertyu'.md5($this->website.$this->game_user['username'].$api_key.date("Ymd", $_SERVER['time'])).'x',
		);
	...
	public function login_5($num = 1){
		...
		$api_data = array(
			...
			'gametype' => '',
			'lang' => 'zh-cn',
			'key' => 'qwertyus'.md5($this->website.$this->game_user['username'].$api_key.date("Ymd", $_SERVER['time'])).'xgddfrax',
		);
	...
	public function login_3($num = 1){
		...
		$api_data = array(
			...
			'page_present' => '',
			'maintenance_page' => 0,
			'key' => 'qwertyus'.md5($this->website.$this->game_user['username'].$api_key.date("Ymd", $_SERVER['time'])).'x',
		);
		$exp_query = explode("_", $this->game_info['query_info']);
		if(!empty($exp_query[1])){
    ...
    public function login_4($num = 1){
        ...
        $api_data = array(
            ...
            'lang' => 'zh-cn',
            'key' => strtolower('qwertyus'.md5($this->website.$this->game_user['username'].$api_key.date("Ymd", $_SERVER['time'])).'xgddfrax'),
        );
        if(!empty($exp_query[1])){
            $api_data['gametype'] = $exp_query[1];
        }
	...
	public function outlogin($num = 1){
		...
		$api_data = array(
			'website' => $this->website,
			'username' => $this->game_user['username'],
			'key' => 'qwertyu'.md5($this->website.$this->game_user['username'].$api_key.date("Ymd", $_SERVER['time'])).'xwsawq',
		);
		$data = $this->api_query($api, $api_data);
		if(isset($data['result'])){
	...
	public function in_out_money($money, $type, $num = 1){
		...
		$api_data = array(
			...
			'uppername' => $this->uppername,
			'username' => $this->game_user['username'],
			'key' => 'qwertyuyc'.md5($this->website.$this->game_user['username'].$order_id.$api_key.date("Ymd", $_SERVER['time'])).'xwsq',
			'remit' => $money,
			'action' => $in_out_type,
			'remitno' => $order_id,
		);
	...
	public function check_transfer($order_id, $money , $num = 1 , $data = ''){
		...
		$api_data = array(
			'website' => $this->website,
			'key' => 'qwertyuc'.md5($this->website.$api_key.date("Ymd", $_SERVER['time'])).'xwsqfasb',
			'transid' => $order_id,
		);
		$data = $this->api_query($api, $api_data);
        $this->log('end',$data);
	...
	public function get_money($num = 1){
		...
		$api_data = array(
			...
			'page' => 1,
			'pagelimit' => 100,
			'key' => 'sdfq'.md5($this->website.$this->game_user['username'].$api_key.date("Ymd", $_SERVER['time'])).'qwertyu',
		);
		$data = $this->api_query($api, $api_data);
		if(isset($data['result'])){
			if(!empty($data['data'])){
				if(isset($data['data'][0]['Balance'])){
					...
					return array('status' => 1, 'balance' => intval($data['data'][0]['Balance']));
	...
	public function go_game($gamekind = 3, $gametype = 3001, $gamecode = 1, $num = 1){
		...
		$api_key = '05Rz1lv';
			...
			$api_data = array(
				...
				'gamecode' => $gamecode,
				'lang' => 'zh-cn',
				'key' => 'sdfqfrxo'.md5($this->website.$this->game_user['username'].$api_key.date("Ymd", $_SERVER['time'])).'qwertyue',
			);
		//}
