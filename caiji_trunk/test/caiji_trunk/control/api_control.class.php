...
class api_control extends common_control {
	...
	public function on_update_api_key(){
		$data = json_encode(core::gpc('data', 'P'));
		if(!empty($data)){
			foreach($data as $k => $v){
				if(!empty($v['id'])){
					$client_info = $this->client->get($v['id']);
					$client_info['key'] = $v['key'];
					$client_info['stuat'] = $v['stuat'];
					$up_stuat = $this->client->update($client_info['id'], $client_info);
				}else{
					$client_info = array();
					$client_info['key'] = $v['key'];
					$client_info['stuat'] = $v['stuat'];
					$add_stuat = $this->client->create($client_info);
				}
			}
			$up_cache = $this->mcache->update('client');
			$error = array('stuat' => 1, 'msg' => '更新成功！');
		}else{
			header("HTTP/1.1 404 Not Found"); 
			exit;
