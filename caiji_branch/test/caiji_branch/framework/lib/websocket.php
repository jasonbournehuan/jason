...
class class_websocket{
	...
	**/
	function run() {
		call_user_func_array( $this->socket->function['get'], array( "", "", "", "" ) );
		echo "213";
		exit;
		...
		while ( true ) {
			...
			// 回调 全部
			empty( $this->socket->function['all'] ) || call_user_func_array( $this->socket->function['all'], array( $this->cycle, $this ) );
	
			
			
			foreach ( $this->cycle as $v ) {
				// 回调 for
				empty( $this->socket->function['for'] ) || call_user_func_array( $this->socket->function['for'], array( $v, $this ) );
				...
				if ( $type === false ) {
					...
					// 回调 add
					WEBSOCKET_TYPE_API == $type || empty( $this->socket->function['add'] ) || call_user_func_array( $this->socket->function['add'], array( $v, $index, $this ) );
					continue;
				}
				...
				if ( WEBSOCKET_TYPE_API == $type ) {
					foreach ( $data as $vv ) {
						if ( empty( $this->socket->function['api'] ) ) {
							$send = false;
						} else {
							$send = call_user_func_array( $this->socket->function['api'], array( $vv, $v, $index, $this ) );;
						}
						$this->send( $send, $v );
					}
				} else {
					foreach ( $data as $vv ) {
						empty( $this->socket->function['get'] ) || call_user_func_array( $this->socket->function['get'], array( $vv, $v, $index, $this ) );
					}
				}
			}
		}
	...
	**/
	function close( $accept ) {
		...
		empty( $this->socket->function['close'] ) || call_user_func_array( $this->socket->function['close'], array( $bind, $this ) );
		return true;
	}
	...
	*	返回值 数组
	**/
	public function parse_header( $html = '', $strtolower = false ) {
		if ( !$html ) {
			return array();
		}
		$html = str_replace( "\r\n", "\n", $html );
	...
	**/
	function header( $data, $accept ) {
		$header = $this->parse_header( $data, true );
		$msg = '';
