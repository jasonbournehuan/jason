...
class cqnc_control extends common_control {
	...
	function __construct() {
		...
		/*
		if($shi >= 3 and $shi < 10){
			$this->on_echo(" ".$this->game_name."3-10点不采集\r\n");
			exit;
		}
		*/
	}
	...
	//采集数据
	public function on_cqncdata(){
		$header = array(
			"content-type: application/json", 
			"timestamp: 1550450057942",
			"sign: 3fd9856f5e48e1a13e5c5911cb38791c",
			"deviceId:h5-Q7OBNMLTUT80OFVPKD90IH1VPN6QR7JC",
		);
		$url = 'https://www.8854a.com/api/v2/trend/queryOpencodes';
		$pdata = array(
			"lotteryId" => 3,
			"time" => '',
		);
		$postData = json_encode($pdata);
		$data = $this->za->curlPost($url, $postData, 3, 600, '', $header);
		print_r($data);exit;
		//采集源ID = 1
		$yid = 1;
